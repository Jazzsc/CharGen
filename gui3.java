...
@Override
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == genB) {
			...
			bg = charStart.bgSelect();
			bgT.setText(bg);

			persona = charStart.personSelect();
			personaA.setText(persona);

			ideal = charStart.idealSelect();
			idealA.setText(ideal);

			bond = charStart.bondSelect();
			bondA.setText(bond);

			flaw = charStart.flawSelect();
			flawA.setText(flaw);

			charStart.statSort(job);
			charStart.raceBuff(race);

			statsA[0].setText(charStart.getStr());
			statsA[1].setText(charStart.getDex());
			statsA[2].setText(charStart.getCon());
...
@Override
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == genB) {
			// System.out.println("button pressed");
			race = charStart.raceSelect();
			raceT.setText(charStart.getRace());

			job = charStart.jobSelect();
			jobT.setText(charStart.getJob());

			bg = charStart.bgSelect();
			bgT.setText(charStart.getBg());

			persona = charStart.personSelect();
			personaA.setText(charStart.getPers());

			ideal = charStart.idealSel(bg);
			idealA.setText(charStart.getIde());

			bond = charStart.bondSel(bg);
			bondA.setText(charStart.getBond());

			flaw = charStart.flawSelect();
			flawA.setText(charStart.getFlaw());
